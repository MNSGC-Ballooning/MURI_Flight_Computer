  ...
  if(millis() - prevTime >= 1400){
    ...
    if (GPS.altitude.isUpdated() || GPS.location.isUpdated()) {
      ...
      if (getGPStime() > lastGPS && newData) {
        ...
        data += (;ng(GPS.altitude.feet())) + ",");    //convert meters to feet for datalogging
        ...
        Serial.println("Printing data string: ");
        Serial.println(data);
        Serial.println("Going into if statements for gps.fix...");
        if (GPS.Fix) { 
          data += "fix,";
